...
def LOOCV(
    ...
    for las_name in las_dict.keys():    
        ...
        for model_name, model in models.items():    
            ...
            Xy_train = pd.concat([las_dict[k] for k in las_dict.keys() if k != las_name], axis=0)
        ...
        print(
            f"Fitted with {model_name} model in {time.time()-time0:.1f} seconds, rmse: {rmse_las:.2f}"
            )        
        ...
        print(f"Mean rmse_las so far: {np.mean(rmse_all_las_temp):.2f}")
